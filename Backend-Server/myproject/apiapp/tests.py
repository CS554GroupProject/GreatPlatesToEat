...
class Request_Unit_Test(unittest.TestCase):
    ...
    def valid_test_for_logRequest(self):
        data = {
            "user_name": "Jawaher Alsaiari",
            "log_test": "This is a valid test",
            "recipes_to_receive": 5,
        }
        request = self.factory.post(reverse("apiapp:log_request"), data)

        with unittest.mock.patch(
            "apiapp.views.create_user_request"
        ) as mock_create_user_request:
    ...
    def invalid_test_for_logRequest(self):
        data = {
            "user_name": "Jawaher Alsaiari",
            "request_text": "This is a valid test",
            "recipes_to_receive": "invalid",
        }
        request = self.factory.post(reverse("apiapp:log_request"), data)
    ...
    def test_create_user_request(self):
        ...
        self.assertEqual(user_request.user, user)
        self.assertEqual(user_request.request, form_data["request_text"])
        self.assertEqual(
            user_request.recipes_to_receive, form_data["recipes_to_receive"]
        )
