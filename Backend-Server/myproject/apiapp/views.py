...
def get_recipe(data: HttpRequest) -> HttpResponse:
    username_object_json = str(data.body, "UTF-8")

    username_object = json.loads(username_object_json)

    storer_manager = RecipeManager()
    requesterFromDatabase = RecipeRequesterFromDatabase(storer_manager)

    if username_object.has_key("name") == False:
        return False

    if (username_object.name is str) == False:
        return HttpResponse("Expected a username for whom to return recipes")

    recipes = requesterFromDatabase.request(username_object.name)
